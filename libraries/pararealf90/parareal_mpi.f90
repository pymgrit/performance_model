...
DOUBLE PRECISION :: timer_coarse, timer_fine, timer_comm, timer_all, T0, T1, my_timings, my_timings2, mea
...
CONTAINS
  ...
  SUBROUTINE PararealMPI(Q_initial, Tend, N_fine, N_coarse, Niter, dx, dy, dz, do_io, be_verbose)
    ...
    IF (myrank==0) THEN
        DO k=1,5
            my_timings = MPI_WTIME()
            CALL Euler(GQ, dt_fine, dt_fine, N_coarse, dx, dy, dz, order_adv_c, order_diff_c)
            mea = mea + MPI_WTIME() - my_timings
        END DO ! End of parareal loop
        print *, 'Measure coarse ', mea / 5
        mea = 0.0
        DO k=1,5
            my_timings = MPI_WTIME()
            CALL Rk3SSp(GQ, dt_fine, dt_fine, N_fine, dx, dy, dz, order_adv_f, order_diff_f)
            mea = mea + MPI_WTIME() - my_timings
        END DO ! End of parareal loop
        print *, 'Measure fine ', mea / 5
    END IF
    ...
    CALL MPI_BARRIER(MPI_COMM_WORLD, ierr)
    print *, 'Runtime ', myrank, timer_all
